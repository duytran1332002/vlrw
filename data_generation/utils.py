...
def load_lyrics(lyrics_file):
    ...
    for i in range(len(words_lines)):
        ...
        assert (offset >= 0)
        assert (new_word == full_lyrics[last_end +
                offset:last_end + offset + len(new_word)])
    ...
    for i in range(len(raw_lines)):
        ...
        assert (offset >= 0)
        assert (line == full_lyrics[last_end +
                offset:last_end + offset + len(line)])
...
def gen_phone_gt(words, raw_lines):
    ...
    for word in words:
        out = g2p(word)
        out = [phone if phone[-1] not in string.digits else phone[:-1]
               for phone in out]
        words_p.append(out)
        if len(lyrics_p) > 0:
            lyrics_p.append(' ')
    ...
    for l in len_words_p:
        ...
        for i in range(len(raw_lines)):
            ...
            for l in line_phone:
                line += l + [' ']
            line = line[:-1]
            line = [phone if phone[-1] not in string.digits else phone[:-1]
                    for phone in line]
            offset = getsubidx(lyrics_p[last_end:], line)
            assert (offset >= 0)
            assert (line == lyrics_p[last_end +
                    offset:last_end + offset + len(line)])
            idx_line_p.append(
                [last_end + offset, last_end + offset + len(line)])
            last_end += offset + len(line)
    ...
    for audio_pos in np.arange(2, audio_length):
        ...
        for ch_pos in np.arange(1, 2*lyrics_length+1):
            ...
            if ch_pos % 2 == 1:  # ch
                ...
                # cur ch -> ch
                a = s[audio_pos-1][ch_pos] + \
                    song_pred[audio_pos][lyrics_int[ch_idx]]
                # last ch -> ch
                b = s[audio_pos-1][ch_pos-2] + \
                    song_pred[audio_pos][lyrics_int[ch_idx]]
                # eps -> ch
                c = s[audio_pos-1][ch_pos-1] + \
                    song_pred[audio_pos][lyrics_int[ch_idx]]
                if a > b and a > c:
