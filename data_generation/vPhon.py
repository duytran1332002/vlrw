...
def trans(word, dialect, chao, eight, nosuper, glottal, phonemic):
    ...
    if length > 0:
        ...
        if nucl in nuclei:
            ...
            # if there is a (non-glottal) onset...
            if ons != 'ʔ':
        ...
        if len(ons) == 2 and ons[1] == lv_gli:
            ons = ons[0]
            gli = lv_gli
        #if ons == 'tʰʷ': ons = 'tʰ'; gli = lv_gli
        if len(ons) == 3 and ons[2] == lv_gli:
            ons = ons[0:2]
            gli = lv_gli
        ...
        tonelist = [tones[word[i]]
                    for i in range(0, length) if word[i] in tones]
        if tonelist:
...
def convert(word, dialect, chao, eight, nosuper, glottal, phonemic, delimit):
    ...
    try:
        (ons, gli, nuc, cod, ton) = trans(
            word, dialect, chao, eight, nosuper, glottal, phonemic)
        if None in (ons, gli, nuc, cod, ton):
            seq = '['+word+']'
        else:
            seq = delimit + \
                delimit.join(filter(None, (ons, gli, nuc, cod, ton)))+delimit
    except TypeError:
        pass
...
def convert_grapheme_to_phoneme(text,
                                chao=False,
                                delimit='',
                                dialect='s',
                                nosuper=False,
    ...
    for i in range(0, len(words)):
        ...
        if (tokenize and '-' in word) or (tokenize and '_' in word):
            ...
            ipa = [convert(x, dialect, chao, eight, nosuper,
                           glottal, phonemic, delimit).strip() for x in values]
            seq = ''.join(v+d for v, d in zip(ipa, delimiters))
        else:
            seq = convert(word, dialect, chao, eight,
                          nosuper, glottal, phonemic, delimit).strip()
        # concatenate
        if len(words) >= 2:
            ortho += ' '
        if i < len(words)-1:
    ...
    phon = ''
    if output_ortho:
        # print(ortho, output_ortho, sep='', end='')
