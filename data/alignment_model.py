...
with warnings.catch_warnings():
    ...
    train_audio_transforms = nn.Sequential(
        torchaudio.transforms.MelSpectrogram(
            sample_rate=sr, n_mels=128, n_fft=n_fft),
    )
...
def data_processing(data):
    ...
    for (waveform, _, _, phone, notes) in data:
        ...
        # convert to Mel
        spec = train_audio_transforms(waveform).squeeze(
            0).transpose(0, 1)  # time x n_mels
        ...
        # the number 3 here and below is due the the maxpooling
    ...
    return (spectrograms, phones,
            input_lengths, phone_lengths, torch.LongTensor(pcs))
...
class AcousticModel(nn.Module):
    ...
    def __init__(self, n_cnn_layers, rnn_dim, n_class, n_feats,
        ...
        self.rescnn_layers = nn.Sequential(*[
            ResidualCNN(n_feats, n_feats, kernel=3, stride=1,
                        dropout=dropout, n_feats=128)
        ...
        self.bilstm = nn.Sequential(
            BidirectionalLSTM(rnn_dim=rnn_dim, hidden_size=rnn_dim,
...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, mat3d, lyrics_gt, melody_gt):
        ...
        # (batch, time, phone, pitch)
        n_batch, n_frame, n_ch, n_p = mat3d.shape
        ...
        # (time, batch, n_ch) reshape for CTC
        y_lyrics = y_lyrics.transpose(0, 1)
        labels, input_lengths, label_lengths = lyrics_gt
        loss_lyrics = self.criterion_lyrics(
            y_lyrics, labels, input_lengths, label_lengths)
...
class BoundaryDetection(nn.Module):
    ...
    def __init__(self, n_cnn_layers, rnn_dim, n_class, n_feats,
        ...
        self.fully_connected = nn.Linear(
            n_feats * 64, rnn_dim)  # add a linear layer
